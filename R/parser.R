...
get_onespecies_count <- function(one_species, reaction_part) {
    terms <- parse_reaction_terms(reaction_part)
    idx <- which(terms$species == one_species)
    if(length(idx) == 0) {
        return(numeric(0))
    }
    return(terms$coeff[idx])
}

#' Parse terms in one side of a reaction.
#'
#' Accepts tokens in the form `aX` or `a X`, where `a` can be integer or
#' decimal (e.g. 2, 1.4, .5). If no coefficient is provided, 1 is assumed.
#' The special token `0` is treated as empty side (formation/degradation).
parse_reaction_terms <- function(reaction_part) {
    part <- trimws(reaction_part)
    if(part == '' || isempty_part(part)) {
        return(data.frame(
            species = character(0),
            coeff = numeric(0),
    ...
    for(tok in tokens) {
        ...
        m <- stringr::str_match(
            tok,
            '^([0-9]+(?:\\.[0-9]+)?|\\.[0-9]+)?\\s*([a-zA-Z][a-zA-Z0-9_]*)$'
        )

        if(is.na(m[1])) {
            warning(
                paste0(
                    "The token '", tok, "' in reaction part '", reaction_part,
        ...
        coeff_txt <- trimws(m[2])
        coeff <- ifelse(is.na(coeff_txt) || coeff_txt == '', 1, as.numeric(coeff_txt))
        if(is.na(coeff)) {
            warning(
                paste0(
                    "Could not parse stoichiometric coefficient in token '", tok,
                    "'. Ignoring token."
                )
            )
            next
        ...
        parsed_species <- c(parsed_species, m[3])
        parsed_coeff <- c(parsed_coeff, coeff)
    }
