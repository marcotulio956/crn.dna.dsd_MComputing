...
Make_NthOrderExponent_Wang <- function(
  name,
  rate,
  cinput1,
  cinput2,
  ...
  # E k5−−→waste
  reactions <- c(
    jn(x, '+', species$intermediate1, '->', x, '+', y, '+', species$intermediate1) ,
    jn(y, '->', species$intermediate2),
    jn(cinput2-1, x, '->', cinput2-1, x, '+',species$intermediate1),
    jn(species$intermediate1, '->', species$intermediate2)
  )

  ki <- c(rate, rate, rate, rate)

  NthOrderExponent_gate <- list(
    name = name,
    species = species,
    reactions = reactions,
    ci = ci,
  ...
  return(NthOrderExponent_gate)
}

Make_AnalogComparator_WTA <- function(
  name,
  rate,   # list: kA1, kA0, kB1, kB0, kann, kout, kamp, kinh
  cin1, cin2, cin3, cin4,  # vector of initial concentrations
  A1,  A0,
  B1,  B0,
  GT,  LT
) {
  ...
  species <- list(
    ...
    P  = jn(name, '_P'),
    N  = jn(name, '_N'),
    fuel1 = jn(name, '_fuel1'),
    fuel2 = jn(name, '_fuel2'),
    GT = GT,
    LT = LT
  )
  # [A1, A0, B1, B0, P, N, GT, LT]
  ci <- c(cin1, cin2, cin3, cin4, 0, 0, 10, 10, 0, 0)
  ...
  reactions <- c(
    ...
    # --- Annihilation (subtraction) ---
    jn(species$P, '+', species$N, '->', '0'),

    # --- Output generation ---
    jn(species$P,'+',species$fuel1,'->', GT),
    jn(species$N,'+',species$fuel1,'->', LT),
    # jn(species$P,'->', GT),
    # jn(species$N,'->', LT),
