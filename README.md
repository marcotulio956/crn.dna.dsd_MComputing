...
# DNAr Development Container

This repository now includes a VS Code Dev Container that installs the R and Julia runtime needed by DNAr.

## What gets installed

- R 4.4.x from the `rocker/r-ver` image.
- The CRAN dependencies listed in `DESCRIPTION`.
- Helpful development packages: `devtools`, `JuliaCall`, and `diffeqr`.
- Julia 1.10.x.
- The Julia packages used by the project: `Suppressor`, `RCall`, `DiffEqBase`, and `DifferentialEquations`.

## CLI

If you prefer Docker directly, build the image with:

```bash
docker build -t dnar-dev -f .devcontainer/Dockerfile .
```

Then start a shell inside it with:
...
```bash
docker exec -it -w /workspaces/dnar dnar-dev R --no-save
```

Inside the container you can start R with `R`, or load the package in development mode with:
