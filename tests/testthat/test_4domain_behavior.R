...
test_that(
    'react_4domain_stochastic returns a seeded stochastic 4-domain result',
    {
        parms <- list(
            species   = c('A', 'B'),
            ci        = c(10, 0),
        ...
        result_1 <- do.call(react_4domain_stochastic, parms)
        result_2 <- do.call(react_4domain_stochastic, parms)

        expect_true(is.list(result_1))
        expect_true(is.data.frame(result_1$behavior))
        expect_equal(result_1$behavior, result_2$behavior)
        expect_equal(names(result_1$behavior), c('time', 'A', 'B'))
    }
)
