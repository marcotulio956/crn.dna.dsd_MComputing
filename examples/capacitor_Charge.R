...
Make_Capacitor_ <- function(name, species_input, species_output, ic) {
  ...
  g1 <- Make_Mul2In_Wang(jn(name, '_mul1'),
    species_input$capacitance, species_input$voltage, species_output$charge,
    ic$capacitance, ic$voltage,
    rate
  )
  gates[[length(gates)+1]] <- g1 
  
  # V_out rate 1=[1/C]*Q_c
  ...
  # Q output = Q_init + Q(v)
  # g6 <- Make_Adder2In_Wang(jn(name, '_add1'),
  #   species_input$charge, Qv, species_output$charge,
  #   ic$charge, 0,
  #   1e3, rate
  # )
...
Plot_behavior(
 ...
 # plot_species=c('c1ol_voltage', 'c1_l_v_rate1','c1ol_current','c1ol_charge'),
  plot_species=c('c1ol_current', 'c1ol_voltage', 'c1_l_v_rate1'),
  timing
)
