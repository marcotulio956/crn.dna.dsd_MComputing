...
plot(time_grid, v_target, type = "l", col = "green", lwd=2, lty = 2,
     ...
     main="Generating a Voltage Source for Step inputs DSD Target=10step(t-10)[V]", xlim=c(5,20), ylim=c(1,10))
lines(time_grid, x1_sim_opt, col = "blue", lwd=3, lty=1)
#lines(time_grid, x2_sim_opt, col = "green", lwd=1, lty=1)
#lines(time_grid, x3_sim_opt, col = "blue", lwd=1, lty=1)

legend("bottomright",
       legend=c("V(S)","v1p"),#,"v2p", "v3p"),
       col=c("green","blue"), lwd=2, lty=c(2,1)) # c("purple","red", 'green', 'blue'), lwd=2, lty=c(2,1, 1, 1))