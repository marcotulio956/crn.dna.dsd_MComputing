...
v_scale <- 10
result_crn['l1ol_vp'] <- result_crn['l1ol_vp'] * v_scale
result_crn['l1ol_vn'] <- result_crn['l1ol_vn'] * v_scale
result_crn['l1ol_v'] <- result_crn['l1ol_vp'] - result_crn['l1ol_vn']
result_crn['l1ol_vp'] <- NULL
result_crn['l1ol_vn'] <- NULL

i_scale <- 0.1
result_crn['l1ol_ip'] <- result_crn['l1ol_ip'] * i_scale
result_crn['l1ol_in'] <- result_crn['l1ol_in'] * i_scale
result_crn['l1ol_i'] <- result_crn['l1ol_ip'] - result_crn['l1ol_in']
result_crn['l1ol_ip'] <- NULL
result_crn['l1ol_in'] <- NULL


simRL <- dnarElectric_simRL(
  timing, result_crn[['l1il_i1p']], 50e-4, 500
)

result_crn['V(L)'] <- simRL$inductor_voltage
result_crn['V(R)'] <- simRL$resistor_voltage
result_crn['I(L,R)'] <- 1e3 * simRL$current_output
...
Plot_behavior(
  ...
  chart_title = 'Inductor Step Response CRN Iin=6.32[mA] L=4.75[mH]',
  timing
)
