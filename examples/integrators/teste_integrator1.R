...
Make_triple_integrator_circuit <- function(cinput1, cinput2) {
  ...
  intg_gate1_oy <- Make_Integrator_OishiYordanov('intg1', 'U', 'y3', 't', 'X',
                                                 'y1p', 'y1n', 0, 30, 1e-3)

  intg_gate2_sub <- Make_Sub2In_Wang('sub1','y1p','y1n','y1',0,0,2,1e-3)

  intg_gate3_oy <- Make_Integrator_OishiYordanov('intg2', 'V', 'y1', 't', 'Y',
                                                 'y2p', 'y2n', 0, 0, 1e-3)

  intg_gate4_sub <- Make_Sub2In_Wang('sub2','y2p','y2n','y2',0,0,2,1e-3)

  intg_gate5_oy <- Make_Integrator_OishiYordanov('intg3', 'W', 'ct', 'y2', 'Z',
                                                 'y3p', 'y3n', 0.075, 0, 1e-3)

  intg_gate6_sub <- Make_Sub2In_Wang('sub3','y3p','y3n','y3',0,0,2,1e-3)
  ...
  circuito <- DNArLogic::circuit_add_gate(circuito, intg_gate2_sub)
  circuito <- DNArLogic::circuit_add_gate(circuito, intg_gate3_oy)
  circuito <- DNArLogic::circuit_add_gate(circuito, intg_gate4_sub)
  circuito <- DNArLogic::circuit_add_gate(circuito, intg_gate5_oy)
  circuito <- DNArLogic::circuit_add_gate(circuito, intg_gate6_sub)
...
circuito <- Make_triple_integrator_circuit(4, 2)
...
Plot_behavior(resultado_crn, circuito, num_gate, minimum, maximum, FALSE)
